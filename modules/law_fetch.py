# law_fetch.py — OC/DRF 비사용(OFF) 안정 버전 (REPLACE)
...
def _extract_text_from_html(html_text: str) -> str:
    try:
        ...
        txt = re.sub(r"<script[\s\S]*?</script>", "", html_text or "", flags=re.I)
        txt = re.sub(r"<style[\s\S]*?</style>", "", txt, flags=re.I)
        txt = re.sub(r"<[^>]+>", "", txt)
    return re.sub(r"\n{3,}", "\n\n", txt).strip()

def _extract_text_from_json(json_text: str) -> str:
    try: data = json.loads(json_text or "{}")
    ...
    def walk(v):
        if isinstance(v, dict):
            ...
            if (al == "Y") and an:
                head = f"제{an}조";  head += f"({at})" if at else ""
                lines.append(head)
            bt = v.get("본문") or v.get("BT")
            if isinstance(bt, str) and bt.strip(): lines.append(bt.strip())
            for k in ("항","호","목","조문","조문내용"): x=v.get(k);  walk(x) if x else None
...
def extract_article_block(full_text: str, art_label: str, max_chars: int=4000) -> str:
    ...
    if not m: return ""
    start = m.start();  n = _ART_HDR.search(full_text, m.end())
...
def fetch_law_detail_text(mst: str, *, prefer: str="JSON",
    ...
    if not USE_DRF: return "", prefer.upper(), ""
